...
function Page() {
  ...
  const slug = searchParams.get("slug"); // ✅ /post_edit?slug=123
  ...
  // ✅ slug 없으면 잘못된 접근 처리 (정적 export에서도 안전)
  useEffect(() => {
    if (!slug) {
      showToast("잘못된 접근입니다. (slug 없음)", "error");
      router.replace("/");
    }
  }, [slug, router]);
          ...
          <div className="edit-image-box">
            <div className="image-fixed-box">
              {isLoading ? (
                ...
                <img src={imageUrl} className="edit-preview-img" alt="preview" />
              ) : (
