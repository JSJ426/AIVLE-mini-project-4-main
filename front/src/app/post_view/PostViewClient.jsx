          ...
          <div className="detail-right">
            ...
            {isOwner && (
              <div className="edit-btn-row">
                <button className="btn-edit me-2" onClick={onEdit}>
                  수정
                </button>
...
export default function PostViewClient() {
  ...
  const slug = searchParams.get("slug"); // ✅ /post_view?slug=123
  ...
  // ✅ 수정 페이지 이동도 쿼리 방식으로
  const editBook = () => router.push(`/post_edit?slug=${slug}`);
  ...
  useEffect(() => {
    if (!slug) {
      window.dispatchEvent(
        new CustomEvent("show-toast", {
          detail: { msg: "잘못된 접근입니다. (slug 없음)", type: "error" },
        })
      );
      router.replace("/");
      return;
